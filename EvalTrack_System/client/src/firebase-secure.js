...
// Never commit actual keys - use .env files instead
...
// These should be set in .env files and NEVER committed to version control
...
const requiredConfigKeys = ['apiKey', 'authDomain', 'projectId', 'storageBucket'];
const missingKeys = requiredConfigKeys.filter(key => !firebaseConfig[key]);

if (missingKeys.length > 0) {
  console.warn(`⚠️ Missing Firebase configuration for: ${missingKeys.join(', ')}`);
  console.warn('Make sure your .env file contains all required VITE_FIREBASE_* variables');
}
...
// Security: Restrict Google authentication to specific domains
